...
public class IngredientStorage extends AppCompatActivity {
    ...
    /**
     * Unique authentication ID for each user generated by Firebase, used to identify documents by
     * user.
     */
    private String id;
    ...
    protected void onCreate(Bundle savedInstanceState) {
                ...
                result -> {
                    ...
                    if (result.getResultCode() == 2) {
                        ...
                        ingredientsDb.document(deleteRef).delete()
                                .addOnSuccessListener(unused -> Log.d("Delete item",
                                        "Delete success"))
                                .addOnFailureListener(e -> Log.d("Delete item",
                                        "Delete failed"));
                    }
                    // When EDIT is chosen
                        ...
                        // Get DocumentReference of the item in the database according to its
                        // position in refList and replace it with the edited object
                        HashMap<String,Object> edited = toHashMap(ingredient);
                        ingredientsDb.document(ingredient.getDocumentID()).update(edited)
                                .addOnSuccessListener(unused -> Log.d("Edit item",
                                        "Edit success"))
                                .addOnFailureListener(e -> Log.d("Edit item",
                                        "Edit failed with Exception"+ e.getMessage()));
                    }
                });

        // onClickListener object for the adapter that listens to when an item in the RecyclerView
        // is clicked on
        ...
        ActivityResultLauncher<Intent> addActivityResultLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == Activity.RESULT_OK) {
                        ...
                        // Add Ingredient object as HashMap to database and put its
                        // DocumentReference in refList
                        DocumentReference addedRef = ingredientsDb.document();
                        addedRef.set(added)
                                .addOnSuccessListener(unused -> Log.d("success",
                                        "Added successfully"))
        ...
        addButton.setOnClickListener(view -> {
            Intent intent = new Intent(IngredientStorage.this,
                    AddEditIngredientActivity.class);
            intent.putExtra("purpose", "ADD");
            addActivityResultLauncher.launch(intent);
        });

        // SnapshotListener to update the list everytime the documents with the user's ID are
        // changed
    ...
    /**
     * This is a helper function to put an {@link Ingredient} object to a {@link HashMap} to send
     * to Firestore database.
     * @param ingredient {@link Ingredient} object to send
     * @return {@link HashMap} containing with the content of the Ingredient as well as the ID of
     * the user
     */
